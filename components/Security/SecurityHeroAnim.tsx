...
import { Shield, Lock, Server, Fingerprint, Database, Scan, Search, Globe, Wifi } from 'lucide-react';
...
const LOG_MESSAGES = {
  ...
  secure: [
    { text: "فایروال: فعال شد", color: "text-luma-yellow" },
    { text: "اتصال ایمن برقرار است", color: "text-luma-yellow" }, // Changed from Green to Yellow
    { text: "تهدیدات مسدود شدند", color: "text-white" }
  ]
};
...
export const SecurityHeroAnim = () => {
  ...
  return (
    <div className="relative w-full h-full bg-[#050505] rounded-[32px] overflow-hidden border border-white/10 shadow-2xl flex flex-col font-sans select-none group" dir="rtl">
       
       {/* --- Header UI --- */}
       <div className="h-14 border-b border-white/5 bg-[#0a0a0a] flex items-center justify-between px-6 shrink-0 z-20">
          <div className="flex items-center gap-3">
             ...
             <div className="flex flex-col">
                <span className="text-[11px] font-bold text-white tracking-wide">سپر امنیتی لوما</span>
       ...
       {/* --- Main Viewport --- */}
       <div className="flex-1 relative overflow-hidden flex items-center justify-center bg-[#030303]">
          ...
          <div className="relative z-10 w-[300px] h-[300px] flex items-center justify-center">
             ...
             <motion.div 
                className="relative z-20 w-32 h-32 bg-[#0a0a0a] rounded-full border flex items-center justify-center shadow-2xl"
                animate={{ 
                ...
                <AnimatePresence mode="wait">
                   {phase === 'scan' && (
                      <motion.div key="scan" initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0, opacity: 0 }}>
                         <Scan size={48} className="text-luma-purple" />
                      </motion.div>
                   )}
                   {phase === 'analyze' && (
                      <motion.div key="analyze" initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0, opacity: 0 }}>
                         <Search size={48} className="text-luma-pink" />
                      </motion.div>
                   )}
                   {phase === 'secure' && (
                      <motion.div key="secure" initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0, opacity: 0 }}>
                         <Shield size={48} className="text-luma-yellow fill-luma-yellow/20" />
                         <motion.div 
                            className="absolute bottom-0 right-0 bg-[#0a0a0a] rounded-full p-1 border border-luma-yellow"
                            initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.2 }}
                         >
                            <Lock size={14} className="text-luma-yellow" />
                         </motion.div>
                      </motion.div>
                   )}
                </AnimatePresence>
          ...
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
             {[
                { icon: Server, color: 'text-luma-purple', delay: 0 },
                { icon: Database, color: 'text-luma-pink', delay: 10 },
                { icon: Globe, color: 'text-luma-yellow', delay: 20 },
                { icon: Wifi, color: 'text-luma-purple', delay: 5 } // Changed Blue to Purple
             ].map((item, i) => (
                <motion.div
                   key={i}
                ...
                >
                   <div className="w-[300px] h-[300px] lg:w-[380px] lg:h-[380px] relative"> {/* Orbit Path Size */}
                      <motion.div 
                         className={`absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 w-10 h-10 rounded-xl bg-[#0a0a0a] border border-white/10 flex items-center justify-center shadow-lg ${phase === 'secure' ? 'border-luma-yellow/50' : ''}`}
                         animate={{ rotate: -360 }} // Counter rotate to keep upright
                         transition={{ duration: 20 + i * 5, repeat: Infinity, ease: "linear", delay: -item.delay }}
                      >
                         {/* Secure state turns icons Yellow instead of Green */}
                         <item.icon size={18} className={phase === 'secure' ? 'text-luma-yellow' : item.color} />
                         {phase === 'secure' && (
                            <motion.div 
                               initial={{ scale: 0 }} 
                               animate={{ scale: 1 }} 
                               className="absolute -top-1 -right-1 w-3 h-3 bg-luma-yellow rounded-full border-2 border-[#0a0a0a]" 
                            />
                         )}
                      </motion.div>
                   </div>
       ...
       {/* --- Footer Terminal --- */}
       <div className="h-40 bg-[#080808] border-t border-white/5 p-5 font-sans text-[11px] leading-relaxed text-gray-400 overflow-hidden relative dir-ltr text-left">
          ...
          <div className="flex flex-col justify-end h-full gap-2 relative z-10 font-sans">
             <AnimatePresence mode='popLayout'>
                {logs.map((log) => (
                   ...
                   >
                      ...
                      <span className="text-luma-purple/50 font-mono">root@luma:~#</span>
             ...
             <div className="flex items-center gap-2">
                <span className="text-gray-600 text-[10px] font-mono">{new Date().toLocaleTimeString('en-US', {hour12: false})}</span>
                <span className="text-luma-purple font-mono">root@luma:~#</span>
                <motion.div 
                   animate={{ opacity: [1, 0] }} 
                   transition={{ duration: 0.8, repeat: Infinity }}
                   className="w-2 h-4 bg-luma-purple"
                />
             </div>
          </div>
       </div>
