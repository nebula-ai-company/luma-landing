    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 animate-in fade-in duration-500 min-h-[80vh]">
      ...
      <aside className="lg:col-span-3 order-2 lg:order-1">
         <div className="sticky top-32 space-y-6">
            <button 
               onClick={onBack}
               className="flex items-center gap-3 text-xs font-bold text-zinc-500 dark:text-gray-400 hover:text-zinc-900 dark:hover:text-white transition-colors mb-8 group w-full px-2"
            >
               <div className="w-8 h-8 rounded-xl bg-zinc-100 dark:bg-white/5 flex items-center justify-center group-hover:bg-zinc-200 dark:group-hover:bg-white/10 transition-colors border border-zinc-200 dark:border-white/5 group-hover:-translate-x-1">
                 <ArrowLeft size={16} />
               </div>
            ...
            <div className="bg-white dark:bg-[#0f0f0f] border border-zinc-200 dark:border-white/5 rounded-2xl p-2 shadow-lg dark:shadow-xl backdrop-blur-sm">
               <div className="px-4 py-3 flex items-center gap-2 border-b border-zinc-100 dark:border-white/5 mb-2">
                  ...
                  {activeCategory.items.map((item) => (
                     <button
                        ...
                        className={`
                           ...
                           ${activePageId === item.id ? 'text-zinc-900 dark:text-white font-bold' : 'text-zinc-500 dark:text-gray-500 hover:text-zinc-900 dark:hover:text-gray-305'}
                        `}
                     >
      ...
      <main className="lg:col-span-9 order-1 lg:order-2">
         {isLoading ? (
            <div className="flex flex-col items-center justify-center py-40 opacity-70 bg-zinc-50 dark:bg-[#0f0f0f] rounded-[32px] border border-zinc-100 dark:border-white/5 min-h-[600px]">
               <div className="relative">
               ...
               <p className="text-xs text-zinc-500 dark:text-gray-500 font-medium mt-4 tracking-widest uppercase">Loading Content...</p>
            </div>
         ) : pageContent ? (
            <motion.div
               key={pageContent.id}
            ...
            >
               ...
               <div className="mb-12 relative">
                  ...
                  <div className="flex items-center gap-3 text-[10px] font-bold uppercase tracking-widest text-luma-purple mb-6">
                     ...
                     <span className="text-zinc-500 dark:text-gray-500">Lesson {currentIndex + 1}</span>
                  </div>
                  
                  <h1 className="text-3xl md:text-5xl lg:text-6xl font-black text-zinc-900 dark:text-white mb-8 leading-tight tracking-tight drop-shadow-sm">
                     {pageContent.title}
                  </h1>
                  
                  <div className="flex items-center gap-6 text-xs text-zinc-500 dark:text-gray-500 font-medium border-b border-zinc-200 dark:border-white/5 pb-8">
                     <div className="flex items-center gap-2">
                        <Calendar size={14} className="text-zinc-450 dark:text-gray-600" />
                        <span>بروزرسانی: ۲۰۲۴</span>
                     </div>
                     <div className="flex items-center gap-2">
                        <Clock size={14} className="text-zinc-450 dark:text-gray-600" />
                        <span>زمان مطالعه: ۵ دقیقه</span>
                     </div>
                     <div className="mr-auto flex items-center gap-2">
                        <button className="p-2 rounded-full bg-zinc-100 hover:bg-zinc-200 dark:bg-white/5 text-zinc-500 dark:text-gray-500 hover:text-zinc-900 group-hover:dark:text-white transition-all duration-300" title="Share">
                           <Share2 size={16} />
                        </button>
                     </div>
                  </div>
                  ...
                  <ReactMarkdown
                     ...
                     components={{
                        ...
                        blockquote: ({node, ...props}) => (
                           <div className="my-10 bg-gradient-to-r from-zinc-100/70 to-zinc-50 dark:from-[#151515] dark:to-[#0f0f0f] border-r-4 border-luma-yellow rounded-xl p-6 relative overflow-hidden shadow-md dark:shadow-lg group">
                        ...
                        img: ({node, ...props}) => (
                           <div className="my-12 rounded-2xl overflow-hidden border border-zinc-200 dark:border-white/10 shadow-lg dark:shadow-2xl bg-[#050505]">
                              <img {...props} className="w-full h-auto block m-0 opacity-90 hover:opacity-100 transition-opacity duration-500" loading="lazy" />
                              {props.title && (
                                 <div className="p-3 bg-zinc-100 dark:bg-[#0f0f0f] border-t border-zinc-200 dark:border-white/5 text-center text-xs text-zinc-500 dark:text-gray-500 font-medium">
                                    {props.title}
                                 </div>
                              )}
                           </div>
                        ...
                        hr: ({node, ...props}) => (
                           <hr className="my-12 border-zinc-200 dark:border-white/5" {...props} />
                        )
                     }}
                  >
               ...
               <div className="mt-24 pt-10 border-t border-zinc-200 dark:border-white/10">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                     ...
                     {prevItem ? (
                       <button 
                           ...
                           className="flex flex-col items-start gap-4 p-6 rounded-3xl border border-zinc-150 dark:border-white/5 bg-white dark:bg-[#0f0f0f] hover:bg-zinc-50 dark:hover:bg-[#151515] hover:border-zinc-300 dark:hover:border-white/10 transition-all group text-right shadow-sm"
                       >
                          <span className="text-[10px] text-zinc-500 dark:text-gray-500 font-bold uppercase tracking-wider flex items-center gap-2 group-hover:text-luma-purple transition-colors">
                             <ChevronRight size={12} /> درس قبلی
                          </span>
                          <span className="text-lg font-bold text-zinc-900 dark:text-white leading-tight group-hover:underline decoration-white/30 underline-offset-4">
                             {prevItem.title}
                          </span>
                       </button>
                     ) : <div />}
                     ...
                     {nextItem ? (
                       <button 
                           ...
                           className="flex flex-col items-end gap-4 p-6 rounded-3xl border border-zinc-200 dark:border-white/10 bg-zinc-50 dark:bg-[#151515] hover:bg-zinc-100 dark:hover:bg-[#1a1a1a] hover:border-luma-purple/30 hover:shadow-lg hover:shadow-luma-purple/5 transition-all group text-left shadow-sm"
                       >
                          <span className="text-[10px] text-zinc-500 dark:text-gray-400 font-bold uppercase tracking-wider flex items-center gap-2 group-hover:text-luma-purple transition-colors">
                             درس بعدی <ChevronLeft size={12} />
                          </span>
                          <span className="text-lg font-bold text-zinc-900 dark:text-white leading-tight group-hover:text-luma-purple transition-colors">
                             {nextItem.title}
                          </span>
                       </button>
                     ) : (
                        <div className="flex flex-col items-center justify-center p-6 rounded-3xl border border-dashed border-zinc-200 dark:border-white/10 bg-zinc-50 dark:bg-white/[0.02]">
                           <span className="text-zinc-500 dark:text-gray-500 text-sm font-medium">پایان دوره</span>
            ...
            <div className="flex flex-col items-center justify-center py-32 border border-dashed border-zinc-200 dark:border-white/10 rounded-3xl bg-zinc-50 dark:bg-white/[0.01] h-full">
               ...
               <p className="text-zinc-500 dark:text-gray-500 text-sm">لطفاً دوباره تلاش کنید یا سرفصل دیگری را انتخاب کنید.</p>
            </div>
         )}
      </main>
    </div>
