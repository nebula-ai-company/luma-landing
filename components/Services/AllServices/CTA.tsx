...
export const CTA: React.FC = () => {
  ...
  const dotStyle = {
    backgroundImage: `url("data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' width='24' height='24' fill='none'%3e%3ccircle cx='1.5' cy='1.5' r='1.5' fill='${theme === 'dark' ? 'rgb(255 255 255 %2f 0.1)' : 'rgb(0 0 0 %2f 0.06)'}'/%3e%3c/svg%3e")`
  };

  return (
    <section ref={containerRef} className="relative py-32 md:py-48 overflow-hidden flex items-center justify-center min-h-[800px] bg-white dark:bg-[#0a0a0a] border-t border-zinc-200 dark:border-white/5 transition-colors duration-300">
      ...
      <div className="absolute inset-0 z-0 overflow-hidden [mask-image:linear-gradient(to_bottom,transparent_0%,black_30%,black_70%,transparent_100%)]">
        ...
        <motion.div 
           ...
           className="absolute top-1/4 left-1/4 w-[600px] h-[600px] bg-luma-purple/10 rounded-full blur-[120px] mix-blend-screen pointer-events-none animate-pulse" 
        />
        <motion.div 
              ...
              {FLOATING_TOOLS.map((item, i) => (
                 ...
                 >
                    <motion.div
                       ...
                       className="flex items-center gap-3 p-3 rounded-2xl bg-zinc-150/55 dark:bg-white/[0.02] border border-zinc-200 dark:border-white/5 backdrop-blur-sm shadow-md dark:shadow-xl transition-all"
                    >
                       <div className={`p-2 rounded-xl bg-zinc-200/50 dark:bg-white/5 ${item.color} opacity-80`}>
                          <item.icon size={28} />
                       </div>
                    </motion.div>
                 </motion.div>
           ...
           <div className="flex flex-col items-center gap-10">
             ...
             >
               <Button 
                 ...
                 variant="primary"
                 className="w-full sm:w-auto px-12 py-4 text-lg shadow-[0_0_40px_-10px_rgba(255,100,130,0.4)] dark:shadow-[0_0_40px_-10px_rgba(255,100,130,0.6)] border-0 ring-1 ring-zinc-300 dark:ring-white/50"
               >
                 شروع کنید
                 <Zap className="w-5 h-5 fill-white dark:fill-black text-white dark:text-black" />
               </Button>
               <Button 
                 href="/pricing" 
                 variant="secondary"
                 className="w-full sm:w-auto px-12 py-4 text-lg bg-zinc-100 dark:bg-white/5 border-zinc-200 dark:border-white/20 hover:bg-zinc-250 dark:hover:bg-white/10 hover:border-zinc-350 dark:hover:border-white/40 text-zinc-800 dark:text-white transition-all duration-300"
               >
                 مشاهده تعرفه‌ها
