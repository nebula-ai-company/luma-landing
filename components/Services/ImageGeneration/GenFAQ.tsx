...
const FAQS: FAQItem[] = [
    ...
    answer: (
      <div className="space-y-2">
        ...
        <ul className="list-disc list-inside space-y-1 text-xs md:text-sm text-zinc-650 dark:text-gray-300">
          <li><strong className="text-luma-purple">FLUX 2 MAX:</strong> مناسب‌ترین گزینه برای تصاویر فوتورئالیستیک، پرتره، مناظر طبیعی و جزئیات سینمایی.</li>
          <li><strong className="text-luma-pink">IDEOGRAM 3:</strong> متخصص درج متن دقیق، طراحی پوستر، تایپوگرافی و لوگوتایپ.</li>
          <li><strong className="text-luma-yellow">RECRAFT 4.1 PRO:</strong> ایده‌آل برای ساخت وکتور، آیکون، گرافیک دوبعدی، استایلیزه و طراحی هویت بصری.</li>
        </ul>
      </div>
    )
  },
  {
    ...
    answer: "تصاویر تولیدی با نسبت‌های تصویر مختلف (۱:۱ مربع، ۱۶:۹ عریض، ۹:۱۶ عمودی، ۴:۳ و...) قابل تنظیم هستند. خروجی با بالاترین وضوح ممکن در فرمت‌های استاندارد PNG و JPG با تراکم پیکسلی بالا قابل دانلود است."
  },
  {
    question: "محاسبه هزینه و سیستم اعتباری (لوم) چگونه کار می‌کند؟",
    answer: (
      <p>
        هزینه ساخت هر تصویر بسته به مدل انتخابی و تنظیمات مشخص می‌شود. اعتبارات به صورت لوم در حساب شما ذخیره می‌شوند و تاریخ انقضا ندارند. قیمت لحظه‌ای هر عملیات به صورت شفاف پیش از ثبت درخواست در پنل کاربری نمایش داده می‌شود. برای مشاهده بسته‌های اعتباری می‌توانید به <Link to="/pricing" className="text-luma-pink font-bold border-b border-luma-pink/40 hover:border-luma-pink inline-flex items-center gap-1 mx-1">صفحه قیمت‌گذاری <ExternalLink size={12} /></Link> مراجعه کنید.
      </p>
    )
  },
  {
    question: "آیا تصاویر تولید شده برای استفاده تجاری مجاز هستند؟",
    answer: "بله، خروجی‌های ساخته شده با حساب شما در لوما، تحت مالکیت و بهره‌برداری شما قرار دارند و می‌توانید از آن‌ها در پروژه‌های شخصی، تجاری، تبلیغات، بسته‌بندی و طراحی وب استفاده کنید."
  }
];
