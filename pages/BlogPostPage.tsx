...
const CodeBlock = ({ code, language = 'bash' }: { code: string, language?: string }) => {
  ...
  return (
    <div className="my-10 rounded-2xl overflow-hidden border border-zinc-200 dark:border-white/10 bg-[#0d0d0d] shadow-xl dark:shadow-2xl relative group dir-ltr text-left">
...
const MediaCard = ({ src, alt, type }: { src: string, alt?: string, type: 'image' | 'video' | 'audio' }) => {
  ...
  return (
    <figure className="my-12 group w-full">
      ...
      {alt && (
         <figcaption className="text-center text-sm text-zinc-550 dark:text-gray-500 mt-3 font-medium">
            {alt}
         </figcaption>
      )}
    </figure>
...
const RelatedPostCard: React.FC<{ item: NavItem }> = ({ item }) => {
  ...
  return (
    <div onClick={handleClick} className="group cursor-pointer">
        <div className="aspect-[4/3] rounded-3xl bg-zinc-100 dark:bg-[#1a1a1a] border border-zinc-200 dark:border-white/5 overflow-hidden mb-6 relative">
            {data?.image ? (
        ...
        <h4 className="text-xl font-bold text-zinc-900 dark:text-white mb-2 group-hover:text-luma-pink transition-colors leading-tight">
            {item.title}
        </h4>
        <p className="text-zinc-500 dark:text-gray-500 text-sm line-clamp-2 leading-relaxed">
    ...
    return (
        <div className="min-h-screen bg-[#FAFAFA] dark:bg-[#0a0a0a] flex items-center justify-center">
            <div className="flex flex-col items-center gap-4">
                ...
                <p className="text-zinc-550 dark:text-gray-500 text-sm animate-pulse">در حال بارگذاری مقاله...</p>
            </div>
        </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#FAFAFA] dark:bg-[#0a0a0a] text-zinc-900 dark:text-white selection:bg-luma-pink selection:text-white font-sans">
      ...
      <div className="relative h-[75vh] w-full overflow-hidden">
         ...
         >
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-[#FAFAFA] via-[#FAFAFA]/70 to-transparent dark:from-[#0a0a0a] dark:via-[#0a0a0a]/60 dark:to-transparent" />
         </motion.div>
         ...
         <div className="absolute inset-0 z-10 flex flex-col justify-end pb-20 px-6">
            <div className="max-w-4xl mx-auto w-full">
                ...
                <motion.button 
                    ...
                    className="mb-8 flex items-center gap-2 text-zinc-600 hover:text-zinc-900 dark:text-white/70 dark:hover:text-white transition-colors group w-fit"
                >
                    <div className="w-8 h-8 rounded-full bg-white/60 dark:bg-white/10 backdrop-blur flex items-center justify-center border border-zinc-200 dark:border-white/10 group-hover:bg-zinc-100 dark:group-hover:bg-white/20 shadow-sm text-zinc-700 dark:text-white">
                        <ChevronLeft size={16} />
                    </div>
                ...
                >
                    <span className="px-3 py-1 rounded-full bg-white/60 dark:bg-white/10 backdrop-blur-md border border-zinc-200 dark:border-white/10 text-xs font-medium text-zinc-700 dark:text-white flex items-center gap-2">
                ...
                >
                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-zinc-950 via-zinc-900 to-zinc-700 dark:from-white dark:via-white dark:to-gray-400">
                        {post.title}
                    </span>
                </motion.h1>
      ...
      <div className="max-w-4xl mx-auto px-6 relative z-20 pb-32">
               ...
               components={{
                  ...
                  h2: ({node, ...props}) => (
                     <div className="mt-16 mb-8">
                        <h2 className="text-3xl font-bold text-zinc-900 dark:text-white flex items-center gap-3 pb-4 border-b border-zinc-200 dark:border-white/10" {...props}>
                  ...
                  hr: ({node, ...props}) => (
                     <hr className="my-16 border-zinc-200 dark:border-white/5" {...props} />
                  ),
                  ...
                  li: ({node, children, ...props}) => (
                     <li className="relative pr-8 text-zinc-800 dark:text-gray-300 leading-9 text-lg" {...props}>
                        <span className="absolute top-3.5 right-0 w-2 h-2 bg-luma-purple rounded-full ring-4 ring-luma-purple/10" />
                        {children}
                     </li>
                  ),
                  blockquote: ({node, children, ...props}) => (
                     <div className="my-12 relative overflow-hidden rounded-3xl bg-gradient-to-br from-zinc-50 to-zinc-100/50 dark:from-[#151515] dark:to-[#0a0a0a] border border-zinc-200 dark:border-white/10 p-8 shadow-md dark:shadow-2xl">
                        <div className="absolute top-0 right-0 p-6 opacity-3 dark:opacity-5 pointer-events-none">
                           <Quote size={80} className="text-zinc-400 dark:text-white" />
                        </div>
                        <div className="absolute left-0 top-8 bottom-8 w-1 bg-luma-yellow rounded-r-full" />
                        <div className="relative z-10 text-xl font-medium text-zinc-805 dark:text-gray-200 italic leading-relaxed pl-6">
                           {children}
                        </div>
                     </div>
                  ),
                  ...
                  code(props) {
                     ...
                     return <code className="bg-zinc-100 dark:bg-white/10 text-luma-pink px-1.5 py-0.5 rounded text-[0.9em] font-mono border border-zinc-200 dark:border-white/5 mx-1" {...rest}>{children}</code>
                  },
                  ...
                  p: ({node, children, ...props}) => {
                     ...
                     // 2. Default Paragraph
                     return <p className="text-zinc-800 dark:text-gray-300 text-lg leading-10 font-light mb-8 text-justify" {...props}>{children}</p>;
                  },
               }}
            >
               {post.content}
         ...
         {/* --- 3. Share & Actions --- */}
         <div className="mt-20 pt-10 border-t border-zinc-200 dark:border-white/10 flex flex-col md:flex-row justify-between items-center gap-6">
            <div className="flex gap-4">
               <button className="flex items-center gap-2 px-6 py-3 rounded-full bg-zinc-100 dark:bg-white/5 hover:bg-zinc-200 dark:hover:bg-white/10 border border-zinc-200 dark:border-white/5 transition-colors text-zinc-800 dark:text-white font-medium">
                  <Share2 size={18} />
                  اشتراک‌گذاری
               </button>
            ...
            <div className="text-zinc-500 dark:text-gray-500 text-sm">
      ...
      {relatedPosts.length > 0 && (
        <section className="py-24 bg-[#f5f5f7] dark:bg-[#050505] border-t border-zinc-200 dark:border-white/5">
            <div className="max-w-6xl mx-auto px-6">
                <h3 className="text-3xl font-bold text-zinc-900 dark:text-white mb-12 flex items-center gap-3">
